...
NAMESPACE="opmon"
CLUSTER="opmon-timescaledb"

echo "--------------------------------------------------"
kubectl get namespaces | grep -q ${NAMESPACE}
if [[ $? -eq 0 ]]; then
    URI_RW=$(
        kubectl -n ${NAMESPACE} get secret ${CLUSTER}-opmon-password -ojsonpath="{.data.uri}" | base64 --decode;
        echo
    )
    URI_RO=$(
    ...
    echo "DUNE DAQ Opmon Service Database URI:"
